...
import pandas as pd
import s3fs
from thresholded_metrics.constants import DATASET
from thresholded_metrics.constants import S3_ANNOTATION_FILE_PATH
from thresholded_metrics.constants import S3_IMAGE_LOCATION
from thresholded_metrics.constants import TEST_SUITE_DESCRIPTION
from thresholded_metrics.constants import TRANSPORTATION_LABELS
from thresholded_metrics.constants import WORKFLOW
...
def load_transportation_data() -> Dict[str, List[LabeledBoundingBox]]:
    ...
    try:
        with open(
            "/Users/diego/Develop/Workspaces/kolena/workspace/kolena/examples/thresholded_metrics/thresholded_metrics/instances_val2014_attribution_2.0_transportation.json",
        ) as file:
            coco_data = json.load(file)
    except OSError as e:
...
def create_complete_transportation_case(args: Namespace) -> TestCase:
    ...
    data = pd.read_csv(
        "/Users/diego/Develop/Workspaces/kolena/workspace/kolena/examples/thresholded_metrics/thresholded_metrics/metadata_attribution2.0_brightness.csv",
    )
...
def main(args: Namespace) -> None:
    ...
    test_suites: List[Tuple[str, Callable[[str, TestCase], TestSuite]]] = [
        (f"NEW {DATASET} :: transportation by brightness [{WORKFLOW}]", seed_test_suite_by_brightness),
        (f"NEW {DATASET} :: transportation by bounding box size [{WORKFLOW}]", seed_test_suite_by_bounding_box_size),
    ]
